...
public class BOJ_2533 {
    ...
    // (트리 구조이기 때문에 자식 노드들의 dp개수를 더해온다)
    static int[][] dp;    // [y][x]일 때,
    // y : 노드 번호,
    // x : 0 -> 해당 노드번호가 earlyAdaptor가 아닐때,
    // 1 -> 해당 노드번호가 earlyAdaptor일 때
    ...
    static void dfs(int number) {
        ...
        dp[number][1] = 1;    // 해당 number가 얼리어답터인 경우
                              // (우선 시작 시 해당 지점 얼리어답터이므로 개수 1)
        ...
        for (int child : graph[number]) {
            if (!visited[child]) {    // dfs 중복 방문 방지(안해도되는데 확인해보기)
                ...
                // 왜냐하면 최소의 얼리어답터 인원을 뽑기 때문에 자식 노드가 얼리어답터 일수도, 
                // 아닐수도 있다.
            }
        }
    }
